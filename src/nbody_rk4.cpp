...
NBodyRK4::NBodyRK4(int n_bodies, int n_dims, double m[], double X_init[],
    ...
    t = 0.0;
    X.resize(2 * n_dims * n_bodies);
    for (int i = 0; i < 2 * n_dims * n_bodies; i++) X[i] = X_init[i];
    X_dot.resize(2 * n_dims * n_bodies, 0.0);
    this->write = write;
    this->p_outfile = p_outfile;

    ri.resize(n_dims);
    rj.resize(n_dims);
    k1.resize(2 * n_dims * n_bodies);
    k2.resize(2 * n_dims * n_bodies);
    k3.resize(2 * n_dims * n_bodies);
    k4.resize(2 * n_dims * n_bodies);
    X_new_1a.resize(2 * n_dims * n_bodies);
    X_new_1b.resize(2 * n_dims * n_bodies);
    X_new_2.resize(2 * n_dims * n_bodies);

    if (!write) print_state();
}

NBodyRK4::~NBodyRK4(void) {
    free(m);
}
...
void NBodyRK4::integrate(double duration, double dt_init, double delta) {
    double dt = dt_init;
    double t_start = t;
